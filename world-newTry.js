...
class BasicWorldDemo {
	...
	TERRAIN_add() {
		...
		for (let i = 0; i < this._terrainSquareIncludedTiles.length; i++) {
			...
			const tile = this._groundTiles2D[tilePos[0]][tilePos[1]];
			tile.userData.wallMesh.userData.updateGeometry();
			tile.userData.updateHeight(this._worldFile.floor.arr[tilePos[0]][tilePos[1]][1]);
		}
		this.SUB_saveWorldFile();
	}
	...
	SUB_createFloor() {
		...
		let makeLittleGroundPlane = (x, z, i, j) => {
			...
			littleSquare.userData.updateHeight = (h) => {
				littleSquare.position.set(x, h * this._squareSize, z);
			}
			littleSquare.userData.updateHeight(height);
			...
			this._groundTiles2D[i].push(littleSquare);

			walls.userData.updateGeometry = (height) => {
		...
		for (let i = 0; i < size; i++) {
			this._groundTiles2D.push(new Array());
