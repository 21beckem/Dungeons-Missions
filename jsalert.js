	...
	(() => {
		...
		class t extends M {
			...
			static prompt(M, N, I, g, D) {
				...
				if ("undefined" == typeof window) return Promise.resolve(console.log("Alert: " + M));
				var s = new t(M, g);
				return s.addButton(i, !0, "default"), s.addButton(e, !1, "cancel"), !1 !== D && s.setIcon(D || t.Icons.Question), s.addTextField(N, null, I), s.show().then((M => s.cancelled ? null : s.getTextFieldValue(0)))
			}
			static loader(M, N) {
			...
			createPopup() {
				this.elems.container = document.createElement("div"), this.elems.container.focusable = !0, this.elems.container.style.cssText = "position: fixed; top: 0px; left: 0px; width: 100%; height: 100%; z-index: 9000000000001; display: flex; justify-content: center; align-items: center; opacity: 0; transform: translateY(-40px); transition: opacity 0.15s, transform 0.15s; ", document.body.appendChild(this.elems.container), setTimeout((() => {
				...
				})), this.elems.window = document.createElement("div"), this.elems.window.classList.add('pixel-corners-4-6'), this.elems.window.style.cssText = "position: relative; background-color: rgba(255, 255, 255, 0.95); box-shadow: 0px 0px 20px rgba(0, 0, 0, 0.25); border-radius: 8px; padding: 10px; min-width: 50px; min-height: 10px; max-width: 80%; max-height: 90%; backdrop-filter: blur(8px); -webkit-backdrop-filter: blur(8px); ", this.elems.container.appendChild(this.elems.window), this.iconURL && (this.elems.icon = document.createElement("img"), this.elems.icon.style.cssText = "display: block; margin: auto; max-height: 40px; text-align: center; font-size: 17px; font-weight: bold; color: #000; cursor: default; padding: 10px 0px; ", this.elems.icon.src = this.iconURL, this.elems.window.appendChild(this.elems.icon)), this.title && (this.elems.title = document.createElement("div"), this.elems.title.style.cssText = "display: block; text-align: center; font-size: 17px; font-weight: bold; color: #000; cursor: default; padding: 2px 20px; ", this.elems.title.innerHTML = D(this.title), this.elems.window.appendChild(this.elems.title)), this.text && (this.elems.text = document.createElement("div"), this.elems.text.style.cssText = "display: block; text-align: center; font-size: 15px; font-weight: normal; color: #000; cursor: default; padding: 2px 20px; ", this.elems.text.innerHTML = D(this.text), this.elems.window.appendChild(this.elems.text)), this.textFields.length > 0 && (this.elems.textFields = document.createElement("div"), this.elems.textFields.style.cssText = "display: block; ", this.elems.window.appendChild(this.elems.textFields), this.textFields.forEach(((M, N) => {
					...
					N.style.cssText = "display: inline-block; font-size: 15px; font-weight: bold; color: var(--light-blue); padding: 10px 20px; padding-bottom: 0px; cursor: pointer; ", N.innerText = M.text, this.elems.buttons.appendChild(N), this.addTouchHandler(N, (() => {
